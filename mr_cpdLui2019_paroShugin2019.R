...
ls()
setwd("c:/arbeit/Publikationen/mr_smoking_alc_paro/data/cpdLui2019noUKBB_paroShugin2019/")
getwd()
mydir <- getwd()
#--------------------
# 1.0) Exposure data: Cigarettes per day, Lui 2019, PMID  30643251, 
#--------------------
cpd<-fread("c:/gwas_summary/smoking/Liu2019/CigarettesPerDay.txt.gz") %>%
  mutate(exposure="cpd") %>%
  dplyr::rename(SNP=RSID)
cpd2<-cpd %>%  format_data(. , type="exposure", chr_col="CHROM", phenotype_col="exposure", snp_col="SNP",
                           beta_col="BETA", se_col="SE", eaf_col="AF",
                           effect_allele_col="ALT",
                           other_allele_col="REF", pval="PVALUE") %>% 
  mutate(rsid=SNP,pval=pval.exposure)
exposure_dat_sel<-subset(cpd2,pval.exposure<5e-8) %>% 
...
r2.xz<-sum(R2_F$R2)
r2.xz*100
expected.or<-c(0.87,0.85,0.8,0.75,0.7,0.65)
...
dat <- harmonise_data(exposure_dat=exposure_dat_sel, outcome_dat=outcome_dat,action=2) %>% 
  mutate(id.outcome = "paro", id.exposure="cpd")
# # # # Export Supplementary Table 1	Association of instruments with cigarettes per day
s_table1_snp_cpd<-dat %>%
  ...
  dplyr::mutate(EAF=round(EAF,3),
                ...
                F=round(F,1))
nrow(s_table1_snp_cpd)
head(s_table1_snp_cpd)
s_table1_snp_cpd
save(s_table1_snp_cpd,file="s_table1_snp_cpd.RData")
s_table1_snp_cpd_X<-s_table1_snp_cpd
s_table1_snp_cpd<-flextable(s_table1_snp_cpd) %>%
  theme_booktabs() %>%
  autofit()
print(s_table1_snp_cpd)
s_table1_snp_cpd <- read_docx() %>%
  body_add_flextable(value = s_table1_snp_cpd)
fileout3 <- tempfile(fileext = ".docx")
fileout3 <- "s_table1_snp_cpd.docx"
print(s_table1_snp_cpd, target = fileout3)
## Export Supplementary Table 2	Association of instruments with paro
s_table2_cpd_snp_paro<-s_table1_snp_cpd_X %>%
  dplyr::select(SNP) %>%
...
head(s_table2_cpd_snp_paro)
nrow(s_table2_cpd_snp_paro)
s_table2_cpd_snp_paro<-flextable(s_table2_cpd_snp_paro) %>%
  theme_booktabs() %>%
  autofit()
print(s_table2_cpd_snp_paro)
s_table2_cpd_snp_paro <- read_docx() %>%
  body_add_flextable(value = s_table2_cpd_snp_paro)
fileout3 <- tempfile(fileext = ".docx")
fileout3 <- "s_table2_cpd_snp_paro.docx"
print(s_table2_cpd_snp_paro, target = fileout3)
#-----------------
# 1.3) Perform MR
#-----------------
...
res
mr_cpd_paro<-res %>%  as.data.frame()  
save(mr_cpd_paro,file="mr_cpd_paro.RData")
##################################
...
het<-mr_heterogeneity(dat, method_list=c("mr_ivw_mre")) %>% 
  ...
  as.data.frame()
het
het_cpd_paro<-het %>%  as.data.frame()  
save(het_cpd_paro,file="het_cpd_paro.RData")
...
plei<-mr_pleiotropy_test(dat) %>% 
  ...
  as.data.frame()
plei
plei_cpd_paro<-plei %>%  as.data.frame()  
save(plei_cpd_paro,file="plei_cpd_paro.RData")
# leave one out
...
res_loo<-res_loo  %>%  
  ...
  setDT()
res_loo
loo_cpd_paro<-res_loo %>%  as.data.frame()  
save(loo_cpd_paro,file="loo_dpd_paro.RData")
