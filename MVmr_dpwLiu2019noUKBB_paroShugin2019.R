...
#' --------- 
#' title: "MVMR Cigarettes per day (Lui2019) and Peridontitis  (SHRINE2019)
...
ls()
setwd("c:/arbeit/Publikationen/mr_smoking_alc_paro/data/dpwLui2019noUKBB_paroShugin2019_MV/")
mydir <- getwd()
#--------------------
...
colnames(bmi)
class(bmi$se)
exposure_dat <- mvmr_extract_exposures_local(exposure_data_list=list(dpw,edu,bmi),
                                             pval_threshold = 5e-06,
                                             clump_r2 = 0.1)
head(exposure_dat)
nrow(exposure_dat)
#rm(list=c("cpd","edu","bmi2"))
#--------------------
...
dat <- mv_harmonise_data(exposure_dat, outcome_dat,harmonise_strictness = 3) 
#-----------------
# 1.3) Perform MR
#-----------------
res <- mv_multiple(dat)  
...
res
mvmr_dpw_paro<-res %>%  as.data.frame()  
save(mvmr_dpw_paro,file="mvmr_dpw_paro.RData")

mvmr_dpw_paro<-flextable(mvmr_dpw_paro) %>%
  theme_booktabs() %>%
  autofit()
print(mvmr_dpw_paro)
mvmr_dpw_paro <- read_docx() %>%
  body_add_flextable(value = mvmr_dpw_paro)
fileout3 <- tempfile(fileext = ".docx")
fileout3 <- "mvmr_dpw_paro.docx"
print(mvmr_dpw_paro, target = fileout3)
