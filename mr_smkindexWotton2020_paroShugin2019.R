...
#' --------- 
#' title: "MR Smoking index (Wotton 2020) and Peridontitis  (SHRINE2019)
...
ls()
setwd("c:/arbeit/Publikationen/mr_smoking_alc_paro/data/smkindexWotton2020_paroShugin2019/")
mydir <- getwd()
#--------------------
# 1.0) Exposure data: Smoking index, Wotton 2020, PMID  31689377,  https://data.bris.ac.uk/data/dataset/10i96zb8gm0j81yz0q6ztei23d
#--------------------
exposure_dat_sel<-fread("c:/gwas_summary/smoking/Wootton2020/2019.10.02 Lifetime Smoking GWAS Data Sheet 2.txt") %>%
  mutate(exposure="smidx") %>%
  format_data(. , type="exposure", chr_col="CHR", phenotype_col="exposure", snp_col="SNP",
...
r2.xz<-sum(R2_F$R2)
r2.xz*100
expected.or<-c(0.93,0.9,0.85,0.8,0.75,0.7,0.65)
...
dat <- harmonise_data(exposure_dat=exposure_dat_sel, outcome_dat=outcome_dat,action=2) %>% 
  mutate(id.outcome = "paro", id.exposure="smidx")
# # # # Export Supplementary Table 1	Association of instruments with smoking index
s_table1_snp_smidx<-dat %>%
  ...
  dplyr::mutate(EAF=round(EAF,3),
                ...
                F=round(F,1))
nrow(s_table1_snp_smidx)
head(s_table1_snp_smidx)
s_table1_snp_smidx
save(s_table1_snp_smidx,file="s_table1_snp_smidx.RData")
s_table1_snp_smidx_X<-s_table1_snp_smidx
s_table1_snp_smidx<-flextable(s_table1_snp_smidx) %>%
  theme_booktabs() %>%
  autofit()
print(s_table1_snp_smidx)
s_table1_snp_smidx <- read_docx() %>%
  body_add_flextable(value = s_table1_snp_smidx)
fileout3 <- tempfile(fileext = ".docx")
fileout3 <- "s_table1_snp_smidx.docx"
print(s_table1_snp_smidx, target = fileout3)
# ## Export Supplementary Table 2	Association of instruments with paro
s_table2_smidx_snp_paro<-s_table1_snp_smidx_X %>%
  dplyr::select(SNP) %>%
...
head(s_table2_smidx_snp_paro)
nrow(s_table2_smidx_snp_paro)
s_table2_smidx_snp_paro<-flextable(s_table2_smidx_snp_paro) %>%
  theme_booktabs() %>%
  autofit()
print(s_table2_smidx_snp_paro)
s_table2_smidx_snp_paro <- read_docx() %>%
  body_add_flextable(value = s_table2_smidx_snp_paro)
fileout3 <- tempfile(fileext = ".docx")
fileout3 <- "s_table2_smidx_snp_paro.docx"
print(s_table2_smidx_snp_paro, target = fileout3)
#-----------------
# 1.3) Perform MR
#-----------------
...
res
mr_smidx_paro<-res %>%  as.data.frame()  
save(mr_smidx_paro,file="mr_smidx_paro.RData")
##################################
...
het<-mr_heterogeneity(dat, method_list=c("mr_ivw_mre")) %>% 
  ...
  as.data.frame()
het
het_smidx_paro<-het %>%  as.data.frame()  
save(het_smidx_paro,file="het_smidx_paro.RData")
...
plei<-mr_pleiotropy_test(dat) %>% 
  ...
  as.data.frame()
plei
plei_smidx_paro<-plei %>%  as.data.frame()  
save(plei_smidx_paro,file="plei_smidx_paro.RData")
# leave one out
...
res_loo<-res_loo  %>%  
  ...
  setDT()
res_loo
loo_smidx_paro<-res_loo %>%  as.data.frame()  
save(loo_smidx_paro,file="loo_smidx_paro.RData")
